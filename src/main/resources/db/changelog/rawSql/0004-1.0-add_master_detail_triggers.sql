...
CREATE OR REPLACE FUNCTION CAZ_PAYMENT_AUDIT.populate_master() RETURNS TRIGGER AS $body$
DECLARE
...
CREATE OR REPLACE FUNCTION CAZ_PAYMENT_AUDIT.populate_detail() RETURNS TRIGGER AS $body$
DECLARE
	master_id uuid;
	ep_latest boolean;
BEGIN
	IF (TG_TABLE_NAME = 't_payment') THEN
	...
	ELSIF (TG_TABLE_NAME = 't_clean_air_zone_entrant_payment') THEN
		SELECT CLEAN_AIR_ZONE_PAYMENT_MASTER_ID INTO master_id FROM T_CLEAN_AIR_ZONE_PAYMENT_MASTER WHERE VRN = NEW.VRN AND CLEAN_AIR_ZONE_ID = NEW.CLEAN_AIR_ZONE_ID;
...
DROP TRIGGER IF EXISTS master_trigger ON CAZ_PAYMENT.T_CLEAN_AIR_ZONE_ENTRANT_PAYMENT;
DROP TRIGGER IF EXISTS detail_trigger_payment ON CAZ_PAYMENT.T_PAYMENT;
DROP TRIGGER IF EXISTS detail_trigger_entrant_payment ON CAZ_PAYMENT.T_CLEAN_AIR_ZONE_ENTRANT_PAYMENT;

CREATE TRIGGER master_trigger
AFTER INSERT OR UPDATE ON CAZ_PAYMENT.T_CLEAN_AIR_ZONE_ENTRANT_PAYMENT
FOR EACH ROW EXECUTE PROCEDURE CAZ_PAYMENT_AUDIT.populate_master();

CREATE TRIGGER detail_trigger_payment
AFTER INSERT OR UPDATE ON CAZ_PAYMENT.T_PAYMENT
FOR EACH ROW EXECUTE PROCEDURE CAZ_PAYMENT_AUDIT.populate_detail();

CREATE TRIGGER detail_trigger_entrant_payment
AFTER INSERT OR UPDATE ON CAZ_PAYMENT.T_CLEAN_AIR_ZONE_ENTRANT_PAYMENT
FOR EACH ROW EXECUTE PROCEDURE CAZ_PAYMENT_AUDIT.populate_detail();