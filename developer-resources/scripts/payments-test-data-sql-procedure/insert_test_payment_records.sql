 ...
 *
 * Before executing - audit table triggers must be disabled.
 */
...
DECLARE
  ...
  _number_of_test_records_progress  integer := 0;
BEGIN


   /*
    * PGAudit logs are disabled here to prevent excess log accrual compromising storage space
    */
   SET pgaudit.log='none';

   WHILE _number_of_test_records_progress <= number_of_test_records
   LOOP
     ...
     BEGIN
		 ...
		 */
		 ...
		 _number_of_test_records_progress := _number_of_test_records_progress + 1;
		
	 EXCEPTION
	   WHEN SQLSTATE '23505' THEN CONTINUE;
     END;
